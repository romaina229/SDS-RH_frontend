...
import { PlusIcon, TrashIcon, FlagIcon, StarIcon } from '@heroicons/react/24/outline';
            ...
            <div className="space-y-6">
                ...
                <div className="flex gap-2 border-b border-gray-200">
                    <button
                        ...
                        className={`px-4 py-2 text-sm font-medium border-b-2 -mb-px ${tab === 'goals' ? 'border-primary-600 text-primary-700' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
                    >
                        Objectifs
                    </button>
                    <button
                        onClick={() => setTab('reviews')}
                        className={`px-4 py-2 text-sm font-medium border-b-2 -mb-px ${tab === 'reviews' ? 'border-primary-600 text-primary-700' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
                    >
                        Évaluations
                    </button>
                </div>
                ...
                {tab === 'goals' && (
                    <div className="space-y-6">
                        ...
                        {showGoalForm && (
                            <Card title="Nouvel objectif" icon={FlagIcon}>
                                <form onSubmit={goalForm.handleSubmit(onSubmitGoal)} className="space-y-4">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="md:col-span-2">
                                            ...
                                            <select {...goalForm.register('employee_id', { required: true })} className="field">
                                        ...
                                        <div className="md:col-span-2">
                                            ...
                                            <input {...goalForm.register('title', { required: true })} type="text" className="field" placeholder="Ex: Augmenter le taux de satisfaction client" />
                                        </div>
                                        <div className="md:col-span-2">
                                            <label className="block text-sm font-medium text-gray-700">Description</label>
                                            <textarea {...goalForm.register('description')} rows={3} className="field" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700">Catégorie</label>
                                            <input {...goalForm.register('category')} type="text" className="field" placeholder="Ex: Commercial, Technique..." />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700">Cible (valeur)</label>
                                            <input {...goalForm.register('target')} type="number" step="0.01" className="field" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700">Priorité</label>
                                            <select {...goalForm.register('priority')} className="field">
                                        ...
                                        <div>
                                            ...
                                            <input {...goalForm.register('start_date', { required: true })} type="date" className="field" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700">Date de fin *</label>
                                            <input {...goalForm.register('end_date', { required: true })} type="date" className="field" />
                                        </div>
                                    </div>
                                                ...
                                                <tr key={goal.id} className="hover:bg-gray-50">
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                        <div className="w-32 bg-gray-200 rounded-full h-2">
                                                            <div className="bg-secondary-500 h-2 rounded-full" style={{ width: `${rate}%` }} />
                                                        </div>
                ...
                {tab === 'reviews' && (
                    <div className="space-y-6">
                        ...
                        {showReviewForm && (
                            <Card title="Nouvelle évaluation" icon={StarIcon}>
                                <form onSubmit={reviewForm.handleSubmit(onSubmitReview)} className="space-y-4">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700">Employé *</label>
                                            <select {...reviewForm.register('employee_id', { required: true })} className="field">
                                                <option value="">Sélectionner un employé</option>
                                                {employeesOptions.map((e) => (
                                                    <option key={e.id} value={e.id}>
                                                        {e.user?.first_name} {e.user?.last_name} ({e.employee_number})
                                        ...
                                        <div>
                                            ...
                                            <input {...reviewForm.register('period', { required: true })} type="text" className="field" placeholder="Ex: 2026-S1" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700">Note globale (/20)</label>
                                            <input {...reviewForm.register('overall_score')} type="number" step="0.5" min={0} max={20} className="field" />
                                        </div>
                                        <div className="md:col-span-2">
                                            <label className="block text-sm font-medium text-gray-700">Points forts</label>
                                            <textarea {...reviewForm.register('strengths')} rows={2} className="field" />
                                        </div>
                                        <div className="md:col-span-2">
                                            <label className="block text-sm font-medium text-gray-700">Axes d'amélioration</label>
                                            <textarea {...reviewForm.register('weaknesses')} rows={2} className="field" />
                                        </div>
                                        <div className="md:col-span-2">
                                            <label className="block text-sm font-medium text-gray-700">Réalisations</label>
                                            <textarea {...reviewForm.register('achievements')} rows={2} className="field" />
                                        </div>
                                        <div className="md:col-span-2">
                                            <label className="block text-sm font-medium text-gray-700">Recommandations</label>
                                            <textarea {...reviewForm.register('recommendations')} rows={2} className="field" />
                                        </div>
                                    </div>
                                    <div className="flex justify-end gap-3 pt-4 border-t">
